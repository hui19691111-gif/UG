...
{
    if (!zhihui_license_guard::EnsureAuthorized(L"ZHIHUI.KONBIAOZU", L"KonBiaoZu"))
    {
        return;
    }
    ...
    {
        UF_initialize();
        KonBiaoZu::KonBiaoZuApplication app;
        app.Run();
        UF_terminate();
    }
    catch (const NXOpen::NXException& ex)
    {
        NXOpen::UI::GetUI()->NXMessageBox()->Show("KonBiaoZu", NXOpen::NXMessageBox::DialogTypeError, ex.Message());
        if (returnCode != nullptr)
        {
    ...
    {
        NXOpen::UI::GetUI()->NXMessageBox()->Show("KonBiaoZu", NXOpen::NXMessageBox::DialogTypeError, "Unhandled exception.");
        if (returnCode != nullptr)
        {
            *returnCode = -1;
        }
