...
int PreUpdateCallback(NXOpen::Features::CustomFeaturePreUpdateEvent* event)
{
    ...
    HMODULE uiModule = GetModuleHandleW(L"CaiPinBan.dll");
    if (uiModule != nullptr)
    {
        BuildCallback build = reinterpret_cast<BuildCallback>(
            GetProcAddress(uiModule, "ZhihuiCaiPinBanBuildCustomFeature"));
        if (build != nullptr)
        {
            return build(event);
        }
    ...
    event->GetCustomFeature()->LogDiagnostic(
        1,
        "The CaiPinBan construction callback is unavailable.",
    ...
    NXOpen::Features::CustomFeatureInformationEvent* event)
{
    event->SetInformation(
        "CaiPinBan: one editable custom feature. Double-click to reopen the dialog.");
    return 0;
}
}
...
extern "C" DllExport void ufusr(char*, int* returnCode, int)
{
    ...
    try
    {
        ...
        NXOpen::Features::CustomFeatureClass* featureClass =
            manager->GetClassFromName(zhihui_caipinban::kFeatureClassName);
