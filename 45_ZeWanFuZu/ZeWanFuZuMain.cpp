...
bool EnsureAuthorized()
{
    ...
    const bool authorized =
        procedure(L"ZHIHUI.CHAIJIJIA", L"折弯辅助板",
                  message, 1024) == 1;
    if (gate.owned) FreeLibrary(gate.module);
    return authorized;
#endif
...
extern "C" DllExport void ufusr(char*, int* returnCode, int)
{
    ...
    try
    {
        ZeWanFuZuDialog dialog;
        dialog.Launch();
    }
    catch (const NXOpen::NXException& ex)
    {
        NXOpen::UI::GetUI()->NXMessageBox()->Show(
            "折弯辅助板", NXOpen::NXMessageBox::DialogTypeError,
            ex.Message());
    ...
    {
        NXOpen::UI::GetUI()->NXMessageBox()->Show(
            "折弯辅助板", NXOpen::NXMessageBox::DialogTypeError, ex.what());
        if (returnCode != nullptr) *returnCode = -1;
    }
    catch (...)
    {
...
extern "C" DllExport void ufusr_cleanup()
{
    ...
    // selections and operation tags are released by ZeWanFuZuDialog's
