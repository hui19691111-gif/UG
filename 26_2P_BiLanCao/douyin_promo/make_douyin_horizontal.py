...
from moviepy import VideoFileClip, ImageClip, CompositeVideoClip, ColorClip, CompositeAudioClip, AudioArrayClip

SRC = r'C:\Users\Administrator\Desktop\302aeb45b1a3df08a553fd1876e47616.mp4'
OUT = r'C:\Users\Administrator\Desktop\zhihui_douyin_horizontal_ready.mp4'
W, H = 1920, 1080
...
def make_wave_silence(duration, sr=44100):
    # Near-silence keeps an audio stream so Douyin can replace/add platform music cleanly.
    samples = np.zeros((int(duration*sr), 2), dtype='float32')
    return AudioArrayClip(samples, fps=sr).with_duration(duration)
...
hint = ImageClip(pill('常用拆件功能集成到工具栏', w=520)).with_duration(min(18,dur)).with_position((690, main_y + 30))
...
cta = ImageClip(text_box((460,64), ['需要的留言'], [34], [(255,255,255,255)], bg=(238,68,45,230), radius=22)).with_start(max(0,dur-10)).with_duration(min(10,dur)).with_position((W-500, main_y + main_h - 144))

final = CompositeVideoClip([bg, main, outline, title, hint, footer, cta] + cap_clips, size=(W,H)).with_duration(dur)

# Keep original audio very low. User should add Douyin hot music on publish page.
audios=[]
if clip.audio is not None:
    try:
        audios.append(clip.audio.with_volume_scaled(0.08))
    except Exception:
        pass
if not audios:
    audios.append(make_wave_silence(dur))
final = final.with_audio(CompositeAudioClip(audios).with_duration(dur))
final.write_videofile(OUT, fps=30, codec='libx264', audio_codec='aac', bitrate='6500k', preset='medium', threads=4)
print(OUT)
