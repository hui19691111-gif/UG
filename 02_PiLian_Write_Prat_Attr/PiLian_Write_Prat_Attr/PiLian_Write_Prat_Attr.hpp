//==============================================================================
//  WARNING!!  This file is overwritten by the Block Styler while generating
//  the automation code. Any modifications to this file will be lost after
//  generating the code again.
//
//       Filename:  D:\LPOpenPrint\application\PiLian_Write_Prat_Attr.hpp
//
//        This file was generated by the NX Block Styler
//        Created by: CHENHUI
//              Version: NX 2000
//              Date: 12-13-2021  (Format: mm-dd-yyyy)
//              Time: 21:26
//
//==============================================================================

#ifndef PILIAN_WRITE_PRAT_ATTR_H_INCLUDED
#define PILIAN_WRITE_PRAT_ATTR_H_INCLUDED

//------------------------------------------------------------------------------
//These includes are needed for the following template code
//------------------------------------------------------------------------------
#include <uf_defs.h>
#include <uf_ui_types.h>
#include <iostream>
#include <NXOpen/Session.hxx>
#include <NXOpen/UI.hxx>
#include <NXOpen/NXMessageBox.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/NXException.hxx>
#include <NXOpen/BlockStyler_UIBlock.hxx>
#include <NXOpen/BlockStyler_BlockDialog.hxx>
#include <NXOpen/BlockStyler_PropertyList.hxx>
#include <NXOpen/BlockStyler_Enumeration.hxx>
#include <NXOpen/BlockStyler_StringBlock.hxx>
#include <NXOpen/BlockStyler_Button.hxx>
#include <NXOpen/BlockStyler_Toggle.hxx>
#include <uf_attr.h>
#include <UF.h>
#include <NXOpen/Part.hxx>
#include <NXOpen/PartCollection.hxx>
#include <NXOpen/AttributeManager.hxx>
#include <NXOpen/AttributePropertiesBaseBuilder.hxx>
#include <NXOpen/AttributePropertiesBuilder.hxx>
#include <NXOpen/Expression.hxx>
#include <NXOpen/ExpressionCollection.hxx>
#include <uf_modl_expressions.h>
#include <uf_ui_ugopen.h>
#include <NXOpen/Unit.hxx>
#include <NXOpen/UnitCollection.hxx>
#include <NXOpen/Expression.hxx>
#include <NXOpen/ExpressionCollection.hxx>
#include <NXOpen/Body.hxx>
#include <NXOpen/BodyCollection.hxx>






//------------------------------------------------------------------------------
// Namespaces needed for following template
//------------------------------------------------------------------------------
using namespace std;
using namespace NXOpen;
using namespace NXOpen::BlockStyler;

class DllExport PiLian_Write_Prat_Attr
{
    // class members
public:
    static Session *theSession;
    static UI *theUI;
    PiLian_Write_Prat_Attr();
    ~PiLian_Write_Prat_Attr();
    int Show();
    
    //----------------------- BlockStyler Callback Prototypes ---------------------
    // The following member function prototypes define the callbacks 
    // specified in your BlockStyler dialog.  The empty implementation
    // of these prototypes is provided in the PiLian_Write_Prat_Attr.cpp file. 
    // You are REQUIRED to write the implementation for these functions.
    //------------------------------------------------------------------------------
    void initialize_cb();
    void dialogShown_cb();
    int apply_cb();
    int ok_cb();
    int update_cb(NXOpen::BlockStyler::UIBlock* block);
    PropertyList* GetBlockProperties(const char *blockID);
    
private:
    const char* theDlxFileName;
    NXOpen::BlockStyler::BlockDialog* theDialog;
    NXOpen::BlockStyler::Enumeration* enum0;// Block type: Enumeration
    NXOpen::BlockStyler::StringBlock* string0;// Block type: String
    NXOpen::BlockStyler::StringBlock* string01;// Block type: String
    NXOpen::BlockStyler::StringBlock* string02;// Block type: String
    NXOpen::BlockStyler::Enumeration* enum01;// Block type: Enumeration
    NXOpen::BlockStyler::Enumeration* enum02;// Block type: Enumeration
    NXOpen::BlockStyler::StringBlock* string03;// Block type: String
    NXOpen::BlockStyler::Toggle* toggleManualQuantity;// Block type: Toggle
    NXOpen::BlockStyler::Toggle* toggleLayerFilter;// Block type: Toggle
    NXOpen::BlockStyler::StringBlock* stringLayerStart;// Block type: String
    NXOpen::BlockStyler::StringBlock* stringLayerEnd;// Block type: String
    NXOpen::BlockStyler::Toggle* toggleAssemblyQuantity;// Block type: Toggle
    NXOpen::BlockStyler::Button* openConfigButton;// Block type: Button
    
};
#endif //PILIAN_WRITE_PRAT_ATTR_H_INCLUDED
