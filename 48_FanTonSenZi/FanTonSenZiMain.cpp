...
bool EnsureAuthorized()
{
    ...
    const bool authorized =
        procedure(L"ZHIHUI.CHAIJIJIA", L"方通伸直",
                  message, 1024) == 1;
    if (gate.owned) FreeLibrary(gate.module);
    return authorized;
#endif
...
extern "C" DllExport void ufusr(char*, int* returnCode, int)
{
    ...
    try
    {
        FanTonSenZiDialog dialog;
        dialog.Launch();
    }
    catch (const NXOpen::NXException& ex)
    {
        NXOpen::UI::GetUI()->NXMessageBox()->Show(
            "方通伸直", NXOpen::NXMessageBox::DialogTypeError,
            ex.Message());
    ...
    {
        NXOpen::UI::GetUI()->NXMessageBox()->Show(
            "方通伸直", NXOpen::NXMessageBox::DialogTypeError, ex.what());
        if (returnCode != nullptr) *returnCode = -1;
    }
    catch (...)
    {
...
extern "C" DllExport void ufusr_cleanup()
{
    ...
    // selections and operation tags are released by FanTonSenZiDialog's
