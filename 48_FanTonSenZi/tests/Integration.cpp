...
void Case(const std::string& file,bool arc,bool inches,bool rotate,double angle=pi/2,bool holes=false){
    ...
    int selectedFaces=0;for(auto* face:dynamic_cast<Body*>(NXObjectManager::Get(f.body))->GetFaces()){try{auto faceSource=InspectFace(face->Tag());Require(std::abs(faceSource.width-s.width)<1e-6*f.u,"face width mismatch");++selectedFaces;}catch(const std::exception&){}}
    Require(selectedFaces==2,"exactly two complete side faces selectable");Require(s.holes.size()==(holes?2:0),"lost or invented holes");Require(std::abs(s.thickness-2*f.u)<1e-5*f.u&&std::abs(s.width-40*f.u)<1e-5*f.u&&std::abs(s.depth-30*f.u)<1e-5*f.u,"automatic dimensions mismatch");
