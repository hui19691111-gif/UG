...
auto* session=NXOpen::Session::GetSession();auto* part=session->Parts()->Work();Plan chosen;int sides=0,planarFaces=0;
for(auto* b:*part->Bodies())if(!b->IsBlanked()&&b->IsSolidBody())for(auto* f:b->GetFaces()){
int type,sign;double pt[3],n[3],box[6],r,r2;Check(UF_MODL_ask_face_data(f->Tag(),&type,pt,n,box,&r,&r2,&sign));if(type!=22)continue;++planarFaces;
...
Require(sides==planarFaces,"Every planar input face must recognize all six holes");double before=Volume(chosen.source.body);auto count=part->Features()->GetFeatures().size();auto mark=session->SetUndoMark(NXOpen::Session::MarkVisibilityVisible,"user model verification");
