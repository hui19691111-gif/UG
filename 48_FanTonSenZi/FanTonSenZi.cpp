...
const char* ids[]={"divisions","radius","kfactor","gap","bridge_width"};
const wchar_t* keys[]={L"Divisions",L"Radius",L"KFactor",L"Gap",L"RoundBridgeWidth"};
const double defaults[]={12,1,.4,.2,6};
...
void FanTonSenZiDialog::Initialize(){
    ...
    try{
        ...
        if(!edges_||!hide_||!cutSource_||!status_||!detail_)throw std::runtime_error("方通伸直对话框资源不完整。");
        ...
        for(int i=0;i<5;++i){numbers_[i]=dialog_->TopBlock()->FindBlock(ids[i]);if(!numbers_[i])throw std::runtime_error("缺少参数控件。");}
...
void FanTonSenZiDialog::Shown(){
    try{if(!initialized_)return;shown_=true;Guard guard(updating_);for(int i=0;i<5;++i)Props(numbers_[i]->GetProperties())->SetDouble("Value",zhihui_dialog_memory::ReadDouble(memory,keys[i],defaults[i]));edges_->Focus();Preview();}
...
tube_straighten::Plan FanTonSenZiDialog::ReadPlan(){
    ...
    double n=Props(numbers_[0]->GetProperties())->GetDouble("Value");if(!std::isfinite(n)||n<2||n>180||std::abs(n-std::round(n))>1e-8)throw std::runtime_error("圆弧分段数须为 2–180 的整数。");
    s.divisions=static_cast<int>(n);s.radiusMm=Props(numbers_[1]->GetProperties())->GetDouble("Value");s.kFactor=Props(numbers_[2]->GetProperties())->GetDouble("Value");s.gapMm=Props(numbers_[3]->GetProperties())->GetDouble("Value");s.bridgeWidthMm=Props(numbers_[4]->GetProperties())->GetDouble("Value");s.cutSource=Props(cutSource_->GetProperties())->GetLogical("Value");s.hideSource=!s.cutSource&&Props(hide_->GetProperties())->GetLogical("Value");if(tags.size()!=1)throw std::runtime_error("请选择方通完整侧平面，或圆管上的一个面。");return tube_straighten::MakePlan(tube_straighten::InspectFace(tags.front()),s);
}
...
void FanTonSenZiDialog::Preview(){
    bool cutSource=Props(cutSource_->GetProperties())->GetLogical("Value");hide_->SetEnable(!cutSource);
    UF_DISP_refresh();if(Select(edges_)->GetSelectedObjects().empty()){Status(cutSource?"原管开槽，同时生成伸直下料件。\n请选择未开槽的原管；间隙须至少为 0.01 mm。":"方通选完整侧平面；圆管选管面或圆环端口。\n自动识别管型；圆管选端口可指定伸直起始端。");return;}auto p=ReadPlan();
    ...
    if(p.source.round)info<<"圆管 Φ"<<p.source.depth/u<<"；连接带弧宽 "<<p.settings.bridgeWidthMm;
    else info<<"方通 "<<p.source.width/u<<" × "<<p.source.depth/u;
    info<<"；壁厚 "<<p.source.thickness/u<<" mm\n伸直长度 "<<p.length/u<<" mm；"<<p.bends.size()<<" 切口；分段偏差 ≤ "<<p.errorMm<<" mm";
...
int FanTonSenZiDialog::Apply(){
    ...
    try{
        ...
        for(int i=0;i<5;++i)zhihui_dialog_memory::SaveDouble(memory,keys[i],numbers_[i]);Select(edges_)->SetSelectedObjects({});UF_DISP_refresh();
        Status(plan.settings.cutSource?"已在原管切间隙槽，并生成伸直下料件。\n原管保持原形状和连接壁厚；可一次撤销两项结果。":(plan.source.round?"已生成圆管伸直实体，外侧连接带保留完整壁厚。\n起始端与原管重合；连接带宽按外圆弧长计。":"已生成方通伸直实体，保留外侧整面连接壁。\n原模型已保留；孔槽随所在段展开，切口避开孔槽。"));edges_->Focus();return 0;
