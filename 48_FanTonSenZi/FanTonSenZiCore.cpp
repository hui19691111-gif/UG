...
namespace
{
void Log(const std::string& message)noexcept{try{std::ofstream(std::filesystem::temp_directory_path()/"Zhihui-TubeCore.log",std::ios::app)<<message<<'\n';}catch(...){}}
    ...
    NXOpen::Features::CustomFeaturePreUpdateEvent* event)
{
    try
    {
        ...
        for (NXOpen::TaggedObject* object : objects)
        {
            ...
            // These members form the straightening and source-slot history. They remain
    ...
    try{
        if (event != nullptr)
            event->SetInformation(
                "方通/圆管伸直：双击编辑原管选面、圆弧分段、连接壁、间隙和 K 因子。原管开槽与伸直下料件由同一特征管理；参考体变化后需编辑确认。\n");
        return 0;
    }catch(const NXOpen::NXException& ex){Log(ex.Message());return 1;}
    ...
    try
    {
        ...
        NXOpen::Features::CustomFeatureClass* featureClass =
            manager->GetClassFromName(
                tube_straighten::featureClassName);
