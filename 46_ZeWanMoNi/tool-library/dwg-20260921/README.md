...
`deploy.py` 将 10 个文件打包在 `D:\UG智辉钣金插件\application\ZeWanMoNiTools` 并更新清单；安装脚本核对其 SHA-256 后复制缺少的文件到 `D:\UG智辉钣金插件\刀图`，同时复制旧用户目录中的自定义刀具并保留原图。已有同名文件会备份并保留原内容，防止覆盖用户在 NX 列表中修改的名称。NX 中重新打开“折弯模拟”或点击“重新加载自定义刀具”，即可看到 `DWG …` 条目。

此操作导入本次图纸的 10 个截面；运行中的插件还没有任意 DWG 一键自动识刀入口。
