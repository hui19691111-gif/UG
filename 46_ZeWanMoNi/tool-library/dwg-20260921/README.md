...
`deploy.py` 将 10 个文件打包在 `D:\UG智辉钣金插件\application\ZeWanMoNiTools` 并更新清单；安装脚本核对其 SHA-256 后复制到 `D:\UG智辉钣金插件\刀图`，同时复制旧用户目录中的自定义刀具并保留原图。已有同名文件先备份到插件 backup 下的时间戳目录，其他用户文件保留。NX 中重新打开“折弯模拟”或点击“重新加载自定义刀具”，即可看到 `自定义：DWG …` 条目。
