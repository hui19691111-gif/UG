...
2026-09-21 刀具列表更新：刀具名称和截面缩略图直接列出；选中后显示截面大图及宽高，并同步更新三维轮廓。自定义刀具使用同一列表。“应用”后保留当前刀具选择。
...
2026-09-23 列表改名：已保存的自定义刀具可在 NX 列表中选中后按 F2 修改名称；名称直接写回 `.ztool`，图形和 DWG 来源记录不变。重新加载保留新名称与选择；安装脚本不覆盖已有用户刀具。

2026-09-23 DWG 默认名称：新导入轮廓保存时直接以 DWG 文件名（含 `.dwg`）为初始刀具名称；预览行另显示图块编号，以便区分同一图纸中的多个候选轮廓。已有刀具名称不变。

2026-09-23 刀具管理：修正中文 DWG 文件名写入 `.ztool` 时的乱码；选中已保存刀具可将定义文件移至 `刀图/已删除`，原 DWG 保留，内置和未保存预览不能删除。移除重复的“重新检查当前姿态”和“重新加载自定义刀具”按钮；选择、换刀、反转时自动检查，外部修改刀具文件后重新打开命令加载。
