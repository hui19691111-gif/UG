...
const std::set<std::pair<int, int> >& GetTubeSpecs()
{
    ...
    {
        LoadSpecsFromFile(moduleDirectory + "\\config\\FanTonGMK_specs.txt", specs);
        LoadSpecsFromFile(moduleDirectory + "\\FanTonGMK_specs.txt", specs);
    }
...
void OpenSpecTable()
{
    ...
    if (moduleDirectory.empty())
    {
        MessageBoxW(NULL, L"Could not resolve FanTonGMK application directory.", L"FanTonGMK", MB_ICONERROR);
        return;
    }

    const std::string path = moduleDirectory + "\\config\\FanTonGMK_specs.txt";
    DWORD attributes = GetFileAttributesA(path.c_str());
    if (attributes == INVALID_FILE_ATTRIBUTES)
    {
        MessageBoxA(NULL, path.c_str(), "FanTonGMK spec table not found", MB_ICONERROR);
        return;
    }
...
void RunFanTonGMK(const std::vector<NXOpen::Body*>& bodies)
{
    NXOpen::Session* session = NXOpen::Session::GetSession();
    ...
    listing->Open();
    listing->WriteLine("FanTonGMK - square tube recognition report");
    listing->WriteLine("");
...
public:
    ...
    {
        ui = NXOpen::UI::GetUI();
        dialog = ui->CreateDialog("FanTonGMK.dlx");
...
private:
    ...
    void SetGuideImageForPortType(int portType)
    {
        ...
        const char* imageName =
            portType == 1 ? "FanTonGMKTaperGuide.bmp" : "FanTonGMKStraightGuide.bmp";
