...
def add_menu(data, ribbon=False):
    ...
    if 'BUTTON  QiuMianZanKai' in text:
        return data
    newline = '\r\n' if '\r\n' in text else '\n'
    block = [' BUTTON  QiuMianZanKai', ' LABEL   球面展开']
    if not ribbon:
        block += [' BITMAP  QiuMianZanKai.bmp', ' ACTIONS QiuMianZanKai']
...
names = ['QiuMianZanKai.dll', 'QiuMianZanKaiCore.dll', 'QiuMianZanKai.dlx', 'QiuMianZanKai.bmp']
...
protection = {name:verify_pe(payload['application/'+name],name=='QiuMianZanKai.dll') for name in names if name.endswith('.dll')}
ns = {'v':'http://schemas.microsoft.com/developer/msbuild/2003'}
for target in ['QiuMianZanKai','QiuMianZanKaiCore']:
...
for rel, data in payload.items():
    ...
    if entries:
        ...
        hashes.append(entry)
command = {
    'launcherName': 'QiuMianZanKai', 'nativeDll': 'QiuMianZanKai.dll',
    'featureCode': 'ZHIHUI.CHAIJIJIA', 'displayName': '球面展开',
    'entryPoint': 'ufusr', 'authorizationGate': 'native-multi-entry',
    'menuButton': 'QiuMianZanKai', 'actionsName': 'QiuMianZanKai',
    'exportsVerified': True, 'sha256': digest(payload['application/QiuMianZanKai.dll']),
    'dlxFiles': 'application/QiuMianZanKai.dlx', 'iconFiles': 'application/QiuMianZanKai.bmp',
    'runtimeFiles': ['application/QiuMianZanKaiCore.dll','application/CustomFeatureConfiguration.xml'],
}
existing = [c for c in package['commands'] if c.get('launcherName') == 'QiuMianZanKai']
assert len(existing) <= 1
if existing:
    existing[0].update(command)
else:
    package['commands'].append(command)
backup = root/'backup'/datetime.datetime.now().strftime('before-QiuMianZanKai_%Y%m%d_%H%M%S')
...
try:
    ...
    verify_pe((root/'application/QiuMianZanKai.dll').read_bytes())
    verify_pe((root/'application/QiuMianZanKaiCore.dll').read_bytes(),False)
    actual_package = json.loads(manifest_paths[1].read_text(encoding='utf-8'))
    assert next(c for c in actual_package['commands'] if c.get('launcherName')=='QiuMianZanKai')['sha256'] == command['sha256']
except Exception:
...
while position < end:
    ...
    if value.get('launcherName') == 'QiuMianZanKai':
...
assert json.loads(raw)['commands'][-1].get('launcherName')=='QiuMianZanKai' or replaced
