...
namespace
{
    ...
    std::string FindConfigPath()
    {
        ...
        paths.push_back(CombinePath(moduleDir, std::string("..\\config\\") + PiLianConfigFileName));
        paths.push_back(CombinePath(moduleDir, PiLianConfigFileName));
        paths.push_back(std::string("D:\\UG智辉钣金插件\\config\\") + PiLianConfigFileName);
    ...
    std::vector<FaceCandidate> GetPlanarCandidates(Body* body)
    {
        ...
        for (size_t i = 0; i < faces.size(); ++i)
        {
            ...
            candidate.type = type;
            candidate.area = FaceAreaScore(faces[i]);
    ...
    bool HasMarkerLineOnFace(Face* face)
    {
        ...
        for (size_t i = 0; i < edges.size(); ++i)
        {
            ...
            double lenA = PointDistance(a1, a2);
            if (lenA <= 2.0)
            {
                continue;
            }
            ...
            for (size_t j = i + 1; j < edges.size(); ++j)
            {
                ...
                double lenB = PointDistance(b1, b2);
                if (lenB <= 2.0)
                {
                    continue;
                }
                double vb[3] = { b2.X - b1.X, b2.Y - b1.Y, b2.Z - b1.Z };
                double parallel = std::fabs((va[0] * vb[0] + va[1] * vb[1] + va[2] * vb[2]) / (vaLen * std::max(lenB, 1e-9)));
                if (parallel < 0.995)
                {
                    continue;
                }
                ...
                if (distance > 1e-9 && distance <= 0.01)
                {
                    return true;
                }
            }
    ...
    FaceInfo SelectConvertBaseFace(Body* body, const AutoConvertOptions& options)
    {
        ...
        {
            FaceInfo markerFace = SelectMarkerLineBaseFace(allCandidates);
            if (markerFace.face != NULL)
            {
                return markerFace;
            }
        }
        ...
        const AutoConvertOptions& options)
    {
        ...
        int innerCount = 0;
        int outerCount = 0;
        for (size_t i = 0; i < tangentFaces.size(); ++i)
        {
            if (tangentFaces[i] == NULL)
            {
                continue;
            }
            if (innerBendTags.count(tangentFaces[i]->Tag()))
            {
                ++innerCount;
            }
            if (outerBendTags.count(tangentFaces[i]->Tag()))
            {
                ++outerCount;
            }
        }
        ...
        {
            return ScoreAndSelectBaseFace(chainCandidates, tangentTags);
        }

        bool currentDirectionIsDownBends = outerCount > innerCount;
        bool desiredDirectionIsDownBends = !options.preferUpBends;
        std::vector<FaceCandidate> directionCandidates = chainCandidates;
        std::map<tag_t, bool> scoringChainTags = tangentTags;
        if (currentDirectionIsDownBends != desiredDirectionIsDownBends)
        {
            directionCandidates.clear();
            std::map<tag_t, bool> seen;
            for (size_t i = 0; i < chainCandidates.size(); ++i)
            {
                bool found = false;
                FaceCandidate parallelFace = FindNearestParallelPlanarFace(
                    allCandidates,
                    chainCandidates[i],
                    chainCandidates[i].area * 0.5,
                    &found);
        ...
        FaceInfo directed = ScoreAndSelectBaseFace(directionCandidates, scoringChainTags);
        return directed.face == NULL ? fallback : directed;
    }
    ...
    int ApplyNeutralFactorByRules(Part* workPart, SheetmetalManager* manager, Body* body, const RuleConfig& ruleConfig)
    {
...
PiLianZuanBanJinDialog::PiLianZuanBanJinDialog()
    : theDialog(NULL),
      theDlxFileName("PiLianZuanBanJin.dlx"),
      strategyGroup(NULL),
      advancedGroup(NULL),
...
int PiLianZuanBanJinDialog::update_cb(UIBlock* block)
{
    if (block == settingsButton)
    {
        ...
        if (exePath.empty())
        {
            ShowMessage(theUI, "自动转钣金 C++", NXMessageBox::DialogTypeInformation, "未找到规则设置程序。");
        }
        else
        {
            ...
            std::string commandLine = "\"" + exePath + "\" " + PiLianSettingsArgument;
            ...
            {
                ShowMessage(theUI, "自动转钣金 C++", NXMessageBox::DialogTypeInformation, "规则设置程序启动失败。");
            }
        }
    }
...
void PiLianZuanBanJinDialog::InitializeValues()
{
    ...
    SetLabel(markerLineFaceUpToggle, "标记线面朝上");
...
void PiLianZuanBanJinDialog::Run(const AutoConvertOptions& options)
{
    ...
    try
    {
        ...
        {
            assemblyMode = true;
            if (!ConfirmAssemblyBatch(theUI))
            {
                return;
            }

            CollectChildPrototypeParts(root, partsToProcess, seenParts);
        }
    }
    catch (...)
    {
        ...
        for (size_t i = 0; i < candidates.size(); ++i)
        {
            ...
            try
            {
                ...
                if (!alreadySheetmetal)
                {
                    ...
                    Feature* convertFeature = convertBuilder->CommitFeature();
                ...
                result.neutralFaceCount = ApplyNeutralFactorByRules(processPart, manager, activeBody, ruleConfig);
                ...
                if (!result.flatOk)
                {
                    result.error = existingFlatPattern == NULL ? "展开未返回有效特征" : "编辑展开未返回有效特征";
                }
            }
            catch (const NXException& ex)
            {
                result.error = "NX执行失败，请检查该实体是否可转钣金并可展开";
            }
            catch (const std::exception& ex)
            {
    ...
    ShowMessage(theUI, "自动转钣金 C++", NXMessageBox::DialogTypeInformation, message);
}
