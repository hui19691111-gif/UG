#pragma once

namespace zhihui_cairban
{
constexpr const char* kFeatureClassName =
    "NXOpen::CustomFeature::CaiRBan";
constexpr const char* kFeatureDisplayName =
    "\xE6\x8B\x86\xE5\x9C\x86\xE5\xBC\xA7\xE6\x9D\xBF";

constexpr const char* kAttrTargetBody = "targetBody";
constexpr const char* kAttrSelectedFace = "selectedFace";
constexpr const char* kAttrRecolorEnabled = "recolorEnabled";
constexpr const char* kAttrColorMode = "colorMode";
constexpr const char* kAttrFixedColor = "fixedColor";
constexpr const char* kAttrAppliedColor = "appliedColor";
constexpr const char* kAttrExtensionLength = "extensionLength";
constexpr const char* kAttrWrapMode = "wrapMode";
}
