...
int PreUpdateCallback(NXOpen::Features::CustomFeaturePreUpdateEvent* event)
{
    ...
    HMODULE uiModule = GetModuleHandleW(L"CaiRBan.dll");
    if (uiModule != nullptr)
    {
        BuildCallback build = reinterpret_cast<BuildCallback>(
            GetProcAddress(uiModule, "ZhihuiCaiRBanBuildCustomFeature"));
        if (build != nullptr)
        {
            return build(event);
        }
    ...
    event->GetCustomFeature()->LogDiagnostic(
        1,
        "The CaiRBan construction callback is unavailable.",
    ...
    NXOpen::Features::CustomFeatureInformationEvent* event)
{
    event->SetInformation(
        "CaiRBan: editable arc-panel extraction feature. Double-click to reopen the dialog.");
    return 0;
}
}
...
extern "C" DllExport void ufusr(char*, int* returnCode, int)
{
    ...
    try
    {
        ...
        NXOpen::Features::CustomFeatureClass* featureClass =
            manager->GetClassFromName(zhihui_cairban::kFeatureClassName);
